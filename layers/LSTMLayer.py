...
class LSTMLayer(torch.nn.Module):
    ...
    def __init__(self, D_in, D_out, n_layers=1, dropout=0, bidirectional=1):
    ...
    def forward(self, x, seq_len, total_length=None, init_hidden=None, sort=False):
        ...
        pack = nn.utils.rnn.pack_padded_sequence(x, seq_len.cpu(), batch_first=True)
        ...
        hidden = hidden.view(self.n_layers, 2 if self.bidirectional else 1, len(seq_len), self.D_out).permute(
            [2, 0, 1, 3])
