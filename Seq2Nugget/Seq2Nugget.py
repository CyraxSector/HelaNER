...
from Seq2Nugget.BoundaryModel import BoundaryModel
from Seq2Nugget.DetectionModel import DetectionModel
from utils.WordDictionary import WordDictionary
from utils.CharDictionary import CharDictionary
from loss.MaxMarginLossLayer import MaxMarginLossLayer
from loss.BagLossWithMarginLayer import BagLossWithMarginLayer
from loss.BagLossLayer import BagLossLayer
...
from Seq2Nugget.Seq2NuggetLogger import Seq2NuggetLogger
...
class Seq2Nugget(object):
    ...
    def initialize(self, train_config, detection_config, boundary_config):
        ...
        self.detection_optimizer = torch.optim.Adadelta(self.detection_model.parameters(),
    ...
    def train(self, train_data, dev_data=None, test_data=None):
        ...
        for epoch in xrange(self.detection_train_epoch):
            ...
            if self.result_logger.is_best_detection():
                logger.info("Better detection performance achieved at epoch %d, saving the detection model." % (
                    self.current_epoch))
        ...
        for epoch in xrange(self.boundary_train_epoch):
            ...
            if self.result_logger.is_best_boundary():
                logger.info("Better Boundary performance achieved at epoch %d, saving the boundary model." % (
                    self.current_epoch))
        ...
        for epoch in xrange(self.joint_train_epoch):
            ...
            if self.result_logger.is_best_overall():
                logger.info("Better Overall performance achieved at epoch %d, saving the boundary model." % (
                    self.current_epoch))
                self.save_best(False, True)
    ...
    def train_boundary_epoch(self, data):
        ...
        for batch in data.mini_batches_for_train(self.batch_size):
            ...
            boundary_loss = self.boundary_loss_fn(left_pred, left_labels, False) + self.boundary_loss_fn(right_pred,
                                                                                                         right_labels,
                                                                                                         False)
    ...
    def test(self, data):
        ...
        for batch in data.mini_batches_for_test(batch_size=100):
            ...
            for sent_idx, (sent_id, sent_cls_pred, sent_len) in enumerate(
                    zip(batch["sent_ids"], cls_outputs, seq_len.tolist())):
    ...
    def restore_detection_model(self, file_name):
        if os.path.exists(file_name + ".detection_model"):
            self.detection_model.load_state_dict(
                torch.load(file_name + ".detection_model", map_location=lambda storage, loc: storage.cuda(self.device)))
    ...
    def restore_boundary_model(self, file_name):
        if os.path.exists(file_name + ".boundary_model"):
            self.boundary_model.load_state_dict(
                torch.load(file_name + ".boundary_model", map_location=lambda storage, loc: storage.cuda(self.device)))
