...
class Seq2NuggetLogger(object):
    ...
    def update_boundary_result(self, data_split, p, r, f1):
        ...
        if data_split == "dev":
            ...
            if self.all_dev_results[self.best_boundary_dev_epoch][-1]:
                if self.detection_dev_results[-1][-1] / f1 <= self.detection_dev_results[self.best_boundary_dev_epoch][
                    -1] / self.all_dev_results[self.best_boundary_dev_epoch][-1]:
        ...
        if data_split == "test":
            ...
            if self.all_test_results[self.best_boundary_test_epoch][-1]:
                if self.detection_test_results[-1][-1] / f1 <= self.detection_test_results[self.best_boundary_test_epoch][
                    -1] / self.all_test_results[self.best_boundary_test_epoch][-1]:
