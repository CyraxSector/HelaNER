...
class Seq2NuggetNERData(NERData):
    ...
    def create_sent_feats(self):
        for words, poss in self.sentences:
            ...
            char_ids, char_len, word_len_from_char = self.char_dict.word2id_translate(words,
                                                                                      max_word_len=self.max_word_len,
                                                                                      max_sent_len=self.max_seq_len)
    ...
    def create_training_data(self):
        ...
        for sent_id, sent_annos in self.annotations.items():
            ...
            left_label = list(range(self.max_seq_len))
            right_label = list(range(self.max_seq_len))
            anns_span = []
