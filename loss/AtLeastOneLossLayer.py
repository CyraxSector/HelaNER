...
class AtLeastOneLossLayer(torch.nn.Module):

    def __init__(self, NIL_index=0):
        super(AtLeastOneLossLayer, self).__init__()
    ...
    def forward(self, y_pred, targets, packages, seq_mask, weight=None):
        ...
        golden_probs = torch.gather(probs, dim=2,
        ...
        max_probs_in_package, _ = torch.max(probs_in_package,
                                            dim=2)

        golden_probs = golden_probs.squeeze(dim=2)
        golden_weight = golden_probs / (
                    max_probs_in_package + self.epsilon)
